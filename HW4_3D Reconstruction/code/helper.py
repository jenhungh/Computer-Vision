...
def epipolarMatchGUI(I1, I2, F):
    ...
    # Create p1s and p2s to store the corresponding points
    p1s = []
    p2s = [] 

    while True:
        plt.sca(ax1)
        # Get the input point on image1
        p1 = plt.ginput(1, mouse_stop=2)
        # Check p1
        if not p1:
            break
        # Extract x1 and y1 
        ...
        # Append p1 and p2 to p1s and p2s
        p1 = [x1, y1]
        p2 = [x2, y2]
        p1s.append(p1)
        p2s.append(p2)
    ...
    np.savez(results_dir+ 'q4_1.npz', F = F, pts1 = p1s, pts2 = p2s)