...
def get_feature_from_wordmap_SPM(opts, wordmap):
    ...
    * hist_all: numpy.ndarray of shape (K*(4^(L+1)-1)/3)
    ...
    # Spatial Pyramid Matching : Change the base of cells and weights from 2 to 3
    # Start from the finest(top) layer
    # Set up the number of cells and size of each cell
    num_cell = 2 ** L              # increase the base to 3
    ...
    if (L == 0 or L == 1):
        weight = 2 ** (-L)         # increase the base to 3     
    else:
        weight = 1/2               # increase the base to 3 
    ...
    for layer_index in range(L-1, -1, -1):
        ...
        if (layer_index == 0 or layer_index == 1):
            ...
            weight = 2 ** (layer_index - L - 1)      # increase the base to 3

        # Aggregate the remaining layers from the finest layer
        num_cell = 2 ** layer_index                  # increase the base to 3
...
def build_recognition_system(opts, n_worker=1):
    ...
    * SPM_layer_num: number of spatial pyramid layers
    ...
    np.savez_compressed(join(out_dir, 'custom_trained_system.npz'), features = features, labels = train_labels,
    dictionary = dictionary, SPM_layer_num = SPM_layer_num)

# Use Intersection Similarity, Eucliden Distance, Kolmogorov-Smirnov Distance, and Match Distance as similarity scores
def distance_to_set(word_hist, histograms):
    ...
    * dis: numpy.ndarray of shape (T)
    ...
    # Euclidean Distance
    # Compute the L2 norm bectween word_hist and histograms
    # T, K = histograms.shape
    # word_hist_all = np.tile(word_hist, (T,1))
    # similarity = np.square(word_hist_all - histograms)
    # Compute the distance
    # dis = np.sum(similarity, axis = 1)
    # dis = np.sqrt(dis)

    # Kolmogorov-Smirnov Divergance
    # Compute the difference bectween word_hist and histograms
    # T, K = histograms.shape
    # word_hist_all = np.tile(word_hist, (T,1))
    # diff = abs(word_hist_all - histograms)
    # Compute the distance 
    # dis = np.amax(diff, axis = 1)
...
def evaluate_recognition_system(opts, n_worker=1):
    ...
    * accuracy: accuracy of the evaluated system
    ...
    data_dir = opts.data_dir
    out_dir = opts.out_dir
    trained_system = np.load(join(out_dir, 'custom_trained_system.npz'))
    ...
    np.savez_compressed(join(out_dir, 'custom_test_system.npz'), features = test_features)
