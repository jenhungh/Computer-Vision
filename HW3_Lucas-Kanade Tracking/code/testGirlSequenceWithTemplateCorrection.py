...
for index in range(num_frames-1):
    ...
    rect_without_correction = np.load('../results/girlseqrects.npy')
    if index in reported_frames:
        ...
        rectangle_wo = patches.Rectangle((x1_wo, y1_wo), (x2_wo - x1_wo), (y2_wo - y1_wo), edgecolor = 'b', facecolor = 'None', linewidth = 3)
        ...
        fig.savefig('../plots/Girl-wcrt_f' + str(index+1) + '.png', bbox_inches = 'tight', pad_inches = 0)

# Save the rectangle history
results_dir = '../results/'
if not os.path.exists(results_dir):
    os.makedirs(results_dir)
np.save('../results/girlseqrects-wcrt.npy', np.asarray(rect_history))