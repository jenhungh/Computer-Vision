...
# Here we also cheat for you and write the forward + backward loop for you
for itr in range(max_iters):
    total_loss = 0
    avg_acc = 0
    for xb,yb in batches:
        # forward
        yb_idx = np.argmax(yb,1)
        ...
        names = ['layer1','output']
        for k in names:
            params['W'+k] -= learning_rate * params['grad_W' + k]
            params['b'+k] -= learning_rate * params['grad_b' + k]
...
print()
print("####################################################")
print("#####       Numerical Gradient Checker    ##########")
print("####################################################")
print()
...
for k,v in params.items():
    ...
    def f(p):
        h1 = forward(x,p,'layer1')
        probs = forward(h1,p,'output',softmax)
        new_loss, _ = compute_loss_and_acc(y, probs)
        return new_loss
    grad_v = np.zeros_like(v)
    it = np.nditer(params[k], flags=['multi_index'], op_flags=['readwrite'])
    while not it.finished:

        # evaluate function at x+h
        ix = it.multi_index
        og = v[ix]
        v[ix] += eps # increment by h
        fxh = f(params) # evalute f(x + h)
        v[ix] -= 2*eps # restore to previous value (very important!)
        fx = f(params) # evalute f(x + h)

        # compute the partial derivative
        grad_v[ix] = (fxh - fx) / (2*eps) # the slope
        v[ix] = og
...
print()
print("####################################################")
print("#####      Numerical Gradient Checker END ##########")
print("####################################################")
print()